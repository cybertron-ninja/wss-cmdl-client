...
context_ptr on_tls_init(const char * hostname, websocketpp::connection_hdl) {
    ...
    try {
		...
		std::cerr << "\nWarning: could not load SSL cert from file ca-chain.cert.pem\n";
    }
    return ctx;
}
...
int main(int argc, char* argv[]) {
    ...
    std::string wss_url = "wss://testnet.bitmex.com/realtime?subscribe=instrument,orderBookL2_25:XBTUSD";

	// get URL string from command line option 1
    if (argc == 2) {
        wss_url = argv[1];
    }

    // extract URL parts using regex
    std::regex r("^(\\w+):\\/\\/([\\w\\d\\.]+)(:)?(\\d*)?\\/([\\w\\d_\\-\\.]+).*");
    std::smatch m;
    std::regex_match(wss_url, m, r);

	// Actual WSS server URI parameters
    protocol = m.format("$1"); // optional, default = "wss"
    hostname = m.format("$2"); // required
    path = m.format("$5"); // required, for example "/realtime?subscribe=instrument,orderBook:XBTUSD"
    port = m.format("$4"); // optional, default = 443
	std::cout << protocol;
	std::cout << hostname;
	std::cout << path;
	std::cout << port;

	// Example of HTTP REST API parameters
	//std::string rest_api_url = "https://jonin2.cybertron.ninja:443/wss/order_book/bitmex/BTC/USD/";
    //rest_hostname = m.format("$2"); // required
    //rest_path = m.format("$5"); // required, for example "/wss/order_book/bitmex/BTC/USD/"
    //rest_protocol = m.format("$1"); // optional, default = "https"
    //rest_port = m.format("$4"); // optional, default = 443
...
//        c.clear_access_channels(websocketpp::log::alevel::frame_payload);
        ...
        client::connection_ptr con = c.get_connection(wss_url, ec);
        if (ec) {
        ...
        c.get_alog().write(websocketpp::log::alevel::app, "Connecting to " + wss_url);
